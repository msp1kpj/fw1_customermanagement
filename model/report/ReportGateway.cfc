component output="false" displayname="Report Gateway" extends="model.base.Gateway" {
	...
	public query function getNoPhoneFull(){
		...
		local.getNoPhone.setSQL("SELECT c1.customerId , c1.firstName , c1.lastName , c1.address , c1.city , c1.phone , s1.dateOfService , s1.description as ServiceDescription , s1.note as ServiceNote , s1.amount , s1.technician FROM customer as c1 LEFT JOIN service as s1 on c1.customerId = s1.customerId AND technician NOT like '%Service Call%' WHERE NULLIF(TRIM(phone), '') is null ORDER BY lastName, firstName, dateOfService;"); //set query
